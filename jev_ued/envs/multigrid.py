...
Ported to gymnasium>=1.0 and minigrid>=3.0 (Farama). Only the 'rgb_array'
render mode is supported, since runs happen on headless machines.
"""
...
from minigrid.core.constants import (COLOR_TO_IDX, DIR_TO_VEC, IDX_TO_COLOR,
                                     IDX_TO_OBJECT, OBJECT_TO_IDX, TILE_PIXELS)
from minigrid.core.grid import Grid as MiniGridGrid
...
class WorldObj(world_object.WorldObj):
  ...
  def decode(type_idx, color_idx, state):
    ...
    if obj_type == 'wall':
      v = world_object.Wall(color)
    elif obj_type == 'floor':
      v = world_object.Floor(color)
    elif obj_type == 'ball':
      v = world_object.Ball(color)
    elif obj_type == 'key':
      v = world_object.Key(color)
    elif obj_type == 'box':
      v = world_object.Box(color)
    elif obj_type == 'door':
...
class Door(world_object.Door):
  """Extends minigrid Door class to multiple agents possibly carrying keys."""

  def toggle(self, env, pos, carrying=None):
    # If the player has the right key to open the door
    if self.is_locked:
      if (isinstance(carrying, world_object.Key) and
          carrying.color == self.color):
...
class Agent(WorldObj):
  ...
  def encode(self):
    ...
    return (OBJECT_TO_IDX[self.type], self.agent_id, self.dir)
...
class MultiGridEnv(gym.Env):
  ...
  def __init__(
    ...
    # Seed used to regenerate the same environment if 'fixed_environment'
    self.seed_value = seed
    self.fixed_environment = fixed_environment

    # Initialize the RNG and the state
  ...
  def reset(self, *, seed=None, options=None):
    ...
    for a in range(self.n_agents):
      ...
      assert (start_cell is None or start_cell.type == 'agent' or
  ...
  def place_obj(self,
    ...
    while True:
      ...
      pos = (self._rand_int(top[0], min(top[0] + size[0], self.grid.width)),
  ...
  def dir_vec(self):
    ...
    return [
        DIR_TO_VEC[self.agent_dir[a]] for a in range(self.n_agents)
    ]
  ...
  def agent_sees(self, x, y, agent_id):
    ...
    obs = self.gen_obs()
    image = obs['image'] if self.minigrid_mode else obs['image'][agent_id]
    obs_grid, _ = Grid.decode(image)
  ...
  def step(self, actions):
    ...
    agent_ordering = self.np_random.permutation(self.n_agents)
    ...
    terminated = self.competitive and np.sum(self.done) >= 1

    # Running out of time applies to all agents
    truncated = self.step_count >= self.max_steps

    return obs, rewards, bool(terminated), truncated, {}
  ...
  def get_obs_render(self, obs, tile_size=TILE_PIXELS // 2):
    ...
    img = grid.render(tile_size, highlight_mask=vis_mask)
  ...
  def render(self, highlight=True, tile_size=TILE_PIXELS):
    """Render the whole-grid view as an RGB array."""
    ...
    # Render the whole grid
    return self.grid.render(tile_size, highlight_mask=highlight_mask)
